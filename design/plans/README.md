...
| [2026-09-08](./2026-09-08-memory-skills-workspace.md) | 长期记忆、技能仓库与统一工作区 | ◐ 部分实施（2026-09-08） | 批次 A 已完成：WorkspaceService（temp/workspaces/{tenant}/{agent}/{conversation} 三级目录 + file/download/pdf 子目录 + shared 跨会话共享，路径守卫复用 ToolSandbox）、ToolFactory 工作区签名（旧签名保留给测试/legacy）、GET /workspace/files 清单端点、manus/files 工作区优先+旧目录回退。待做：批次 B markdown 技能仓库、批次 C 分层记忆（beforeThink 压缩 hook + 事实日志 V7 + 提取/注入 hook） |
