...
public class TestAgentApp {
    ...
    public TestAgentApp(ChatModel dashscopeChatModel, AgentKnowledgeRagService knowledgeRagService,
                        AgentConversationService conversationService, AgentRegistry agentRegistry) {
    ...
    public Flux<String> chat(String tenantId, String conversationId, String message, Long retryUserMessageId,
        ...
        return chatClient.prompt().system(agentRegistry.get(KEY).systemPrompt() + "\n" + retrieval.context() + "\n最近对话：" + history)
                .user(message).stream().content().doOnNext(answer::append).doOnComplete(() -> {
